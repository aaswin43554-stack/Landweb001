...
export function FieldOfficerScreen({ onTriggerP2PSync }: FieldOfficerScreenProps = {}) {
  ...
  function loadData() {
    setIsLoading(true)
    Promise.all([fetchDisputes(), fetchVillages(), getCachedParcels()]).then(([d, v, p]) => {
      setDisputes(d)
      setVillages(v)
      setParcels(p)
      setIsLoading(false)
  ...
  async function handleSaveAction() {
    ...
    if (success) {
      ...
      alert('Case Resolution updated. The citizen can now view it.')
      loadData()
    } else {
  ...
  const simulatedWaypoints = useMemo(() => {
    ...
    const cachedParcel = parcels.find(p => p.id === parcelId)
        ...
        <div className="flex items-center justify-between max-w-2xl sm:max-w-3xl md:max-w-5xl mx-auto w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">Case Resolution & Spatial Auditing Panel</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1 bg-slate-700 text-white border-2 border-slate-600 px-3 py-1.5 rounded-xl font-bold text-xs hover:bg-slate-650 transition-all cursor-pointer shadow-sm animate-pulse"
            >
              📷 Scan QR
            </button>
            <button
